...
struct DebugMenuLine fogMenu[8] = {
	...
	{ DEBUG_LINE_TYPE_ACTION, 0, 0, "RESTORE FOG", (long*)&DEBUG_FogRestore, (long)0 },
	{ DEBUG_LINE_TYPE_ENDLIST, 0, 0, "", (long*)0, 0 },
};
...
void DEBUG_DisplayStatus(struct GameTracker* gameTracker)//Matching - 58.54%
{
	int deg;
	long numberInQueue;
	...
	if ((gameTracker->debugFlags & 0x4000004) != 0)
	{
		if ((gameTracker->debugFlags & 0x4000000) != 0)
		{
		...
		{
			if (gameTracker->idleTime)
			{
				FONT_Print(" IDLE %d PCT\n", gameTrackerX.idleTime * 100 / 33333);
			}
			else
			{
		...
		deg = (theCamera.focusPointAccl.z >= 0x801 ? (4096 - theCamera.focusPointAccl.z) : -(theCamera.focusPointAccl.z)) * 360;
		FONT_Print(" CAM TILT %d DIST %d\n", deg < 0 ? (deg + 0xFFF) >> 12 : deg >> 12);
	}

	if (gameTracker->debugFlags < 0)
	{
		if (STREAM_IsCdBusy(&numberInQueue))
		{
...
void DEBUG_EndViewVram(struct GameTracker* gameTracker)//Matching - 88.50%
{
...
{ 
#if 0//TODO find out where this checkagain is coming from.
	while (checkagain() != 0)
	{
