...
{
	unsigned int oldbaseaddr; // changed type in regards to SYMDUMP
	unsigned int* rel_addr; // changed type in regards to SYMDUMP
	...
	while (*relocs != -1)
	{
		...
		{
		case 0:
			*rel_addr = *rel_addr + offset;
			break;
		case 1:
			*(short*)rel_addr = (((baseaddr + *relocs++) + 32768) >> 16);
			break;
		case 2:
			*(short*)rel_addr = *rel_addr + offset;
			break;
		case 3:
