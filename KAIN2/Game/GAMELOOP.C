...
void MainRenderLevel(struct _StreamUnit* currentUnit, unsigned long** drawot)
{
	...
	MATRIX cam_mat_save; // stack offset -88
	...
	//a0 = gameTrackerX.core.MorphTime
	...
	lw      $v0, 0x48($s4)
		...
		lh      $v0, 0x1A($s0)
		nop
		bltz    $v0, loc_8002EFFC
		nop
		lhu     $v1, 0x12($s0)
		nop
		andi    $v0, $v1, 1
		bnez    $v0, loc_8002EFFC
		andi    $v0, $v1, 0x40
		beqz    $v0, loc_8002EF44
		move    $s2, $zero
		lw      $s2, -0x422C($gp)
		sw      $zero, -0x422C($gp)
	...
	addiu   $a0, $sp, 0x50 + var_30
		addiu   $a1, $sp, 0x50 + var_10
		lhu     $a3, 0x50 + var_38($sp)
		lhu     $v0, 0xC($s0)
		lhu     $v1, 0x50 + var_38 + 2($sp)
		lw      $a2, -0x5310($gp)
		subu    $a3, $v0
		addiu   $a2, 0x14
		sh      $a3, -0x5370($gp)
	...
	lw      $v0, 0x44($s4)
		...
		lh      $a3, -0x45B4($gp)
		addiu   $t1, $gp, -0x5370
		ulw     $t2, 0x50 + var_38($sp)
		lh      $t3, 0x50 + var_34($sp)
		usw     $t2, 0($t1)
		sh      $t3, 4($t1)
		lw      $v0, -0x5310($gp)
		addiu   $a1, $gp, -0x5370
		lw      $t2, 0x50 + var_30($sp)
		lw      $t3, 0x50 + var_2C($sp)
		lw      $t4, 0x50 + var_28($sp)
		lw      $t1, 0x50 + var_24($sp)
		sw      $t2, 0($v0)
		sw      $t3, 4($v0)
		sw      $t4, 8($v0)
		sw      $t1, 0xC($v0)
		lw      $t2, 0x50 + var_20($sp)
		lw      $t3, 0x50 + var_1C($sp)
		lw      $t4, 0x50 + var_18($sp)
		lw      $t1, 0x50 + var_14($sp)
		sw      $t2, 0x10($v0)
		sw      $t3, 0x14($v0)
		sw      $t4, 0x18($v0)
		sw      $t1, 0x1C($v0)
	...
	addiu   $t3, $gp, -0x45AC
		lw      $sp, 0($t3)
		nop
		lw      $ra, 0x50 + var_s24($sp)
		lw      $fp, 0x50 + var_s20($sp)
		lw      $s7, 0x50 + var_s1C($sp)
		lw      $s6, 0x50 + var_s18($sp)
		lw      $s5, 0x50 + var_s14($sp)
		lw      $s4, 0x50 + var_s10($sp)
...
{ // line 1999, offset 0x8002ee58
	/* begin block 1 */
		...
		// Variables:
			...
			//MATRIX cam_mat_save; // stack offset -80
...
void GAMELOOP_DisplayFrame(struct GameTracker* gameTracker)
{
	...
	if (!(gameTrackerX.gameFlags & 0x8000000) || pause_redraw_flag != 0)
	{
		...
		//loc_8002F90C
		//a0 = 
		//v0 = mainStreamUnit->StreamUnitID
		//v1 = 0x140
		...
		{
			d = 0;
			//s7 = ((short*)mainLevel->terrain->StreamUnits + 17);
			//loc_8002F9D4

			//t0 = ((int*)mainLevel->terrain->StreamUnits + 6)[0];
			//s3 = ((int*)mainLevel->terrain->StreamUnits + 11)[0];
			toStreamUnitID = ((int*)mainLevel->terrain->StreamUnits + 6)[0];
			if (((int*)mainLevel->terrain->StreamUnits + 11)[0] == 0 ||
				((int*)((int*)mainLevel->terrain->StreamUnits + 11)[0])[8] != gameTrackerX.displayFrameCount)
			{
				//loc_8002F9FC
...
// autogenerated function stub: 
// MATRIX * /*$ra*/ GAMELOOP_GetMatrices(int numMatrices /*$a0*/)
