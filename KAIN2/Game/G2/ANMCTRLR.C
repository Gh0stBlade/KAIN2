...
struct _G2AnimController_Type* _G2AnimControllerST_FindInList(int segNumber, int type, unsigned short* listPtr)
{
	...
	while (controller > &_controllerPool.blockPool[0])
	{
		...
		{
			if (controller == &_controllerPool.blockPool[0])
			{
				return NULL;
			}
			else
...
struct _G2AnimController_Type* _G2AnimControllerST_RemoveFromList(int segNumber, int type, unsigned short* listPtr)
{
	...
	while (&_controllerPool.blockPool[0] < controller)
	{
		if (controller->segNumber == segNumber && controller->type == type)
		{
			...
			if (&_controllerPool.blockPool[0] < controller)
			{
				stepController = nextController;

				listPtr[0] = nextController->next;

				nextController = &_controllerPool.blockPool[nextController->next];

				if (&_controllerPool.blockPool[0] >= nextController || nextController->type != 0)
				{
