...
void _G2AnimSection_TriggerEffects(struct _G2AnimSection_Type *section, short startTime, short endTime)
{
	...
	while (fxHeader != NULL && fxHeader->type != -1)
	{
		...
		{
			if (startTime < (short)(keylist->s0TailTime * fxHeader->keyframeID) || endTime >= (short)(keylist->s0TailTime * fxHeader->keyframeID) && ((short)(keylist->s0TailTime * fxHeader->keyframeID) >= startTime && endTime <= (short)(keylist->s0TailTime * fxHeader->keyframeID)))
			{
				if (section->callback != NULL)
				{
					Anim = _G2AnimSection_GetAnim(section);
					section->callback(Anim, section->sectionID, G2ANIM_MSG_PLAYEFFECT, fxHeader->type, (long)(fxHeader + 1), (struct _Instance*)section->callbackData);
				}
			}
		}
