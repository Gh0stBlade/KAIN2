...
int MON_SetUpKnockBack(struct _Instance* instance, struct _Instance* enemy, struct evMonsterHitData* data)  // Matching - 100%
{
	...
	mv = (struct _MonsterVars*)instance->extraData;
	t = data->knockBackDistance;
	d = data->knockBackDuration;
	a = PHYSICS_FindAFromDAndT(t, d);
	v = PHYSICS_FindVFromAAndD(a, t);
	*(short*)&front = MATH3D_AngleFromPosToPos(&instance->position, &enemy->position);
	if (((((unsigned short)instance->rotation.z - front) + 1024) & 0xFFF) <= 2047)
	{
	...
	*((unsigned int*)&mv->destination.y) = MON_GetTime(instance) + d * 33;
	return result;
}
