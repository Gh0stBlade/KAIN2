...
void MON_CheckEnvironment(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterVars* mv;
	struct _MonsterAttributes* ma;
	int inwater;
	struct _SVector Hack;
	...
	if (instance->currentMainState != 29)
	{
		Hack.z = 0;
		Hack.y = 0;
		Hack.x = 0;
		...
		else
		{
			...
			{
				MON_CheckTerrainAndRespond(instance, &((struct Level*)(instance->tfaceLevel))->terrain->BSPTreeArray[instance->bspTree], instance->tface);
			}
		}
...
void MON_EnableHeadMove(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterAttributes* ma;
...
void MON_DisableHeadMove(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterAttributes* ma;
...
void MON_LookInDirection(struct _Instance* instance, short tx, short tz) // Matching - 100%
{
...
void MON_LookAtPos(struct _Instance* instance, struct _Position* position) // Matching - 100%
{
	struct _MonsterVars* mv;
	short z;
...
void MON_ProcessLookAt(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterVars* mv;
	int temp;  // not from SYMDUMP
	...
	if (mv->mode != 0x80000)
	{
		...
		if ((mv->lookAngleX != 0) || (mv->lookAngleZ != 0))
		{
...
int MON_ShouldIFlee(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterVars* mv;
	...
	if (enemy != NULL)
	{
		if (!(enemy->mirFlags & 0x8))
		{
			if (enemy->distance < mv->subAttr->fleeRange)
			{
...
void MON_ProcessSpecialFade(struct _Instance* instance) // Matching - 99.48%
{
	struct _MonsterVars* mv;
	...
	if ((instance->object->oflags & 0x80000))
	{
		if ((instance->currentMainState != 23) && (instance->currentMainState != 16))
		{
			...
			else if (gameTrackerX.gameData.asmData.MorphTime >= 667)
			{
			...
			else if (instance->fadeValue >= 4097)
			{
				instance->fadeValue = 4096;
			}
	...
	else
	{
		...
		if ((mv->auxFlags & 0x8000000))
		{
			fadeRate = mv->fadeRate;

			targetFadeValue = mv->targetFade;

			temp2 = fadeRate * gameTrackerX.timeMult;

			currentFadeValue = instance->fadeValue + (temp2 / 4096);

			if (fadeRate > 0)
			{
				temp3 = targetFadeValue < currentFadeValue;
			}
			else
			{
				temp3 = currentFadeValue < targetFadeValue;
			}
