...
void MON_DeadEntry(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterVars* mv;
...
void MON_Flee(struct _Instance* instance) // Matching - 100%
{
	struct _MonsterVars* mv;
	...
	if (((mv->mvFlags & 0x2000000)) && (mv->ally != NULL))
	{
		if (mv->ally->distance < mv->enemy->distance)
		{
