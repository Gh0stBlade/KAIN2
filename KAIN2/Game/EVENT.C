...
void EVENT_RelocateInstanceList(struct Level* oldLevel, struct Level* newLevel, long sizeOfLevel)  // Matching - 99.84%
{
	...
	struct EventWildCardObject* wildcardEventObject;  // not from SYMDUMP
	struct EventInstanceObject* instanceEventObject;  // not from SYMDUMP
	struct EventVmObject* vmEventObject;  // not from SYMDUMP
		...
		for (i = 0; i < puzzle->numPuzzles; i++)
		{
			...
			for (i2 = 0; i2 < puzzle->eventInstances[i]->numInstances; i2++)
			{
				...
				else if (basicEventObject[i2]->id == 6)
				{
					...
					if (IN_BOUNDS(wildcardEventObject->objectName, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						wildcardEventObject->objectName = (char*)OFFSET_DATA(wildcardEventObject->objectName, offset);
					}
				}
				else if (basicEventObject[i2]->id == 1)
				{
					...
					if (IN_BOUNDS(instanceEventObject->data.intro, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						instanceEventObject->data.intro = (struct Intro*)OFFSET_DATA(instanceEventObject->data.intro, offset);
					}
				}
				// disabled this block of code because struct EventVmObject is unlikely to be what basicEventObject 
				// needs to be parsed to.
				/*else if (basicEventObject[i2]->id == 7)
				{
					vmEventObject = (struct EventVmObject*)basicEventObject[i2];

					if (IN_BOUNDS(vmEventObject[1].attribute, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						vmEventObject[1].attribute = OFFSET_DATA(vmEventObject[1].attribute, offset);
					}
				}*/
			}
		}
