...
void LitShaftInit(struct _Instance* instance, struct GameTracker* gameTracker) // Matching - 100%
{
...
void EVENT_UpdateResetSignalArrayAndWaterMovement(struct Level* oldLevel, struct Level* newLevel, long sizeOfLevel)  // Matching - 100%
{
	long offset;
	long i;
...
void EVENT_ProcessPuppetShow(struct Event* eventInstance, long startIndex)  // Matching - 100%
{
	/*	enable this code for cutscenes
	long i;
	...
	if (!(eventInstance->actionList[startIndex]->conditionBits & 2))
	{
		...
		{
			eventInstance->processingPuppetShow = (int)(i + 2);
...
void EVENT_AddObjectToStack(struct _PCodeStack* stack, long item)  // Matching - 100%
{
	struct EventBasicObject* basicObject;
	int topOfStack;
	struct StackType* stackEntry;
	...
	if (topOfStack < 32)
	{
		...
		{
		case 5:
			if (((struct EventSignalObject*)basicObject)->mSignal != NULL)  // uses custom made struct
			{
				stackEntry->id = 17;

				stackEntry->data.signalObject.msignal = ((struct EventSignalObject*)basicObject)->mSignal;
			...
			break;
		case 3:
			if (((struct EventTGroupObject*)basicObject)->bspTree != NULL)
			{
				stackEntry->id = 23;

				stackEntry->data.terrainGroup.bspTree = ((struct EventTGroupObject*)basicObject)->bspTree;
				stackEntry->data.terrainGroup.streamUnit = ((struct EventTGroupObject*)basicObject)->stream;
		...
		case 1:
			...
			stackEntry->data.listObject.eventInstance = (struct EventWildCardObject*)basicObject;
			stackEntry->data.listObject.numberOfAttributes = 0;
			stackEntry->data.listObject.lineID = CurrentEventLine;

			break;
		case 2:
			EVENT_WriteEventObject(stackEntry, ((struct EventEventObject*)basicObject)->unitID, ((struct EventEventObject*)basicObject)->event, ((struct EventEventObject*)basicObject)->eventNumber);
			break;
		case 4:  // correct parsing for stackEntry, basicObject needs double-checking
			stackEntry->id = 1;

			stackEntry->data.areaObject.streamUnit = (struct _StreamUnit*)((struct EventWildCardObject*)basicObject)->objectName;
			stackEntry->data.areaObject.attribute = -1;
			stackEntry->data.areaObject.unitID = ((struct EventWildCardObject*)basicObject)->unitID;

			break;
		case 0:
			if ((((struct EventInstanceObject*)basicObject)->flags & 1))
			{
				stackEntry->id = 27;

				stackEntry->data.soundObject.flags = 1;
				stackEntry->data.soundObject.data.sfxMarker = ((struct EventInstanceObject*)basicObject)->data.sfxMarker;
			...
			{
				if (((struct EventInstanceObject*)basicObject)->instance != NULL)
				{
					stackEntry->id = 2;

					stackEntry->data.instanceObject.instance = ((struct EventInstanceObject*)basicObject)->instance;
					stackEntry->data.instanceObject.attribute = -1;

					break;
				}

				if (((struct EventInstanceObject*)basicObject)->data.intro != NULL)
				{
					stackEntry->id = 4;

					stackEntry->data.introObject.intro = ((struct EventInstanceObject*)basicObject)->data.intro;
			...
			break;
		case 6:
			if (((struct EventEventVmObject*)basicObject)->vmObject != NULL)  // uses custom made struct
			{
				stackEntry->id = 26;

				stackEntry->data.vmObject.vmObjectPtr = ((struct EventEventVmObject*)basicObject)->vmObject;

				stackEntry->data.vmObject.level = STREAM_GetLevelWithID(((struct EventEventVmObject*)basicObject)->unitID);
...
long EVENT_GetGameValue(struct GameObject* gameObject)  // Matching - 100%
{
	long value;
...
long EVENT_TransformSavedEventAttribute(struct _PCodeStack* stack, struct StackType* stackObject, long item, short* codeStream)  // Matching - 100%
{
	long retValue;
	long offset;
	struct SavedBasic* temp;  // not from SYMDUMP
		...
		if (offset < 6U)
		{
			...
			if (temp != NULL)
			{
				if (temp->savedID == 2)
				{
					...
					EVENT_AddShortPointerToStack(stack, &((struct SavedEvent*)temp)->eventVariables[offset]);
				...
				EVENT_AddCharPointerToStack(stack, &((struct SavedEventSmallVars*)temp)->eventVariables[offset]);
...
long EVENT_TransformListObjectAttribute(struct _PCodeStack* stack, struct StackType* stackObject, long item)  // Matching - 100%
{
	long retValue;
	int temp;  // not from SYMDUMP
...
long EVENT_TransformSplineAttribute(struct _PCodeStack* stack, struct StackType* stackObject, long item, short* codeStream)  // Matching - 100%
{
	long retValue;
	struct _Instance* instance;
	long maxKeyFrames;
	...
	case 31:
		...
		else
		{
			...
			{
				instance->clipEnd = (int)maxKeyFrames;
			}
		...
		break;
	case 34:
		stackObject->data.instanceSpline.splineFlags |= 4;

		retValue = 1;
		break;
	case 35:
		stackObject->data.instanceSpline.splineFlags |= 8;
...
long EVENT_TransformIntroAttribute(struct _PCodeStack* stack, struct StackType* stackObject, long item)  // Matching - 100%
{
	long retValue;
	long x;
	long y;
	...
	case 5:
		...
		else
		{
			...
			EVENT_ChangeOperandVector3d(stackObject, (int)x, (int)y, (int)z, STREAM_WhichUnitPointerIsIn(intro)->StreamUnitID);
		}
...
long EVENT_ParseOperand2(struct StackType* operand2, long* error, long* trueValue)  // Matching - 100%
{
	long number;
...
long EVENT_DoVMObjectAction(struct EventVmObject* vmobject, struct StackType* operand2)  // Matching - 100%
{
	long result;
	long trueValue;
	long number;
	...
	if (vmobject->attribute != -1)
	{
		...
		case 13:
			if (trueValue != 0)
			{
				vmobject->vmObjectPtr->flags &= 0xFFFD;
			}
			else
			{
				vmobject->vmObjectPtr->flags |= 2;
			}
...
long EVENT_DoGameAction(struct GameObject* gameObject, struct StackType* operand2)  // Matching - 100%
{
	long error;
	long number;
	long trueValue;
	...
	if (gameObject->attribute != -1)
	{
		...
		case 119:
			...
			break;
		case 142:
			HINT_StartHint((int)number);
			break;
		case 156:
			HINT_KillSpecificHint((int)number);
			break;
		case 147:
			HINT_StopHint();
			break;
...
long EVENT_DoSignalAction(struct SignalObject* signalObject, struct StackType* operand2)  // Matching - 99.35%
{
	long trueValue;
	long number;  // unused but present in SYMDUMP
	long error;
...
long EVENT_TransformSignalAttribute(struct _PCodeStack* stack, struct StackType* stackObject, long item)  // Matching - 100%
{
	long retValue;
	...
	case 26:
		...
		break;
	case 50:
		EVENT_ChangeOperandToNumber(stackObject, msignal->flags & 1, 0);
...
long EVENT_TransformVector3dAttribute(struct _PCodeStack* stack, struct StackType* stackObject, long item, short* codeStream)  // Matching - 100%
{
	long retValue;
...
long EVENT_DoInstanceAnimateTextureAction(struct InstanceAnimateTexture* instanceAniTexture, struct StackType* operand2)  // Matching - 100%
{
	long trueValue;
	long number;
	long result;
	...
	else
	{
		...
		case 40:
			if (error == 0)
			{
				instanceAniTexture->instance->currentTextureAnimFrame = (int)number;
			}
...
long EVENT_DoSplineAction(struct InstanceSpline* instanceSpline, struct StackType* operand2)  // Matching - 99.88%
{
	long trueValue;
	long number;
	long result;
	...
	else
	{
		...
		case 40:
			if (number != -1)
			{
				SCRIPT_InstanceSplineSet(instance, (int)number, NULL, NULL, NULL);
		...
		case 28:
		{
			...
			{
				instance->splineFlags |= 1;

				instance->targetFrame = (int)number;

				instance->flags |= 0x2000000;

				if (number < curKeyFrame)
				{
					instanceSpline->splineFlags &= 4;
...
long EVENT_DoAnimateAction(struct InstanceAnimate* instanceAnimate, struct StackType* operand2) // Matching - 100%
{
	long trueValue;
	long number;
	long result;
	...
	if (instanceAnimate->attribute != -1)
	{
		...
		case 121:
			break;
		case 132:
			instance->aliasCommand.speed = (short)number;
			break;
		case 30:
			result = 1;
			...
			if (number != -1)
			{
				instance->aliasCommand.newanim = (short)number;
			}
		...
		case 40:
			if (number != -1)
			{
				instance->aliasCommand.newframe = (short)number;
			}
		...
		case 102:
			if (number != -1)
			{
				instance->aliasCommand.interpframes = (short)number;
			}
		...
		case 41:
			if (trueValue != 0)
			{
				...
				{
					temp2 = SetAnimationInstanceSwitchData(instance, instance->aliasCommand.newanim, instance->aliasCommand.newframe, instance->aliasCommand.interpframes, 1);
				...
				{
					temp2 = SetActionPlayHostAnimationData(instance, instance->aliasCommand.hostInstance, instance->aliasCommand.newanim, instance->aliasCommand.newframe, instance->aliasCommand.interpframes, 1);
				...
				INSTANCE_Post(instance2, temp, temp2);
			}
			else
			{
			...
			memset(&instance->aliasCommand.hostInstance, 0, 12);
		...
		case 42:
			...
			{
				INSTANCE_Post(instance, 0x40003, SetActionPlayHostAnimationData(instance, instance->aliasCommand.hostInstance, instance->aliasCommand.newanim, instance->aliasCommand.newframe, (int)instance->aliasCommand.interpframes, 2));
			}
		...
		case 107:
			...
			{
				INSTANCE_Post(instance, 0x40003, SetActionPlayHostAnimationData(instance, instance->aliasCommand.hostInstance, instance->aliasCommand.newanim, instance->aliasCommand.newframe, (int)instance->aliasCommand.interpframes, 0));
			}
...
long EVENT_GetTGroupValue(struct TGroupObject* terrainGroup)  // Matching - 100%
{
	long value;
	long trueValue;
	struct BSPTree* bspTree;
...
long EVENT_DoTGroupAction(struct TGroupObject* terrainGroup, struct StackType* operand2)  // Matching - 100%
{
	long trueValue;
	long number;
	long error;
	...
	if (terrainGroup->attribute != -1)
	{
		...
		case 44:
		{
			...
			if (curWater != NULL)
			{
				...
				curWater->oldWaterZ = (int)terrainGroup->streamUnit->level->waterZLevel;
		...
		case 47:
		{
			...
			if (bspTree->localOffset.z != terrainGroup->arg[0])
			{
				...
				bspTree->globalOffset.z = (bspTree->globalOffset.z + (int)offsetz);
				...
				{
					terrainGroup->streamUnit->level->waterZLevel = (terrainGroup->streamUnit->level->waterZLevel + offsetz);
					terrainGroup->streamUnit->level->terrain->UnitChangeFlags |= 3;
				}
			}
...
long EVENT_DoObjectSoundAction(struct SoundObject* soundObject, struct StackType* operand2)  // Matching - 100%
{
	long trueValue;
	long number;
	long error;
	...
	{
		struct SoundInstance* soundInstance;
...
long EVENT_DoSoundMarkerAction(struct SoundObject* soundObject, struct StackType* operand2)  // Matching - 100%
{
	long trueValue;
	long number;
	long error;
...
long EVENT_DoIntroAction(struct IntroObject* introObject, struct StackType* operand2)  // Matching - 100%
{
	long trueValue;
	long number;
	long error;
		...
		case 62:
		{
			...
			for (i = 0; i < 16; i++)
			{
				...
				if (temp->used == 2)
				{
					...
					for (j = temp->level->numIntros; j != 0; j--)
					{
						if (intro1 == intro)
						{
			...
			if (INSTANCE_IntroduceInstance(intro, id) == 0)
			{
				EventAbortLine = 1;
			}
...
void EVENT_DoStackOperationEquals(struct _PCodeStack* stack, short* codeStream) // Matching - 100%
{
...
// goto labels below need getting rid of or at least reduced in amount 
long EVENT_CompareVector3d(struct Vector3d* vector1, struct Vector3d* vector2)  // Matching - 100%
{
	long retValue;
	...
	if (vector1->streamUnitID == vector2->streamUnitID)
	{
		if (vector1->errory != -1)
		{
			if (vector1->errorz != -1)
			{
				if ((vector2->vx >= (vector1->vx - vector1->errorx)) && ((vector1->vx + vector1->errorx) >= vector2->vx)
					&& ((vector2->vy >= (vector1->vy - vector1->errory)) && ((vector1->vy + vector1->errory) >= vector2->vy)
						&& (vector2->vz >= (vector1->vz - vector1->errorz))))
				{
					long dist;

					dist = ((vector1->vz + vector1->errorz) < vector2->vz) ^ 1;
			...
			if (vector1->errory != vector1->errorz)
			{
				if ((POW2(vector2->vx - vector1->vx) + POW2(vector2->vy - vector1->vy)) < POW2(vector1->errorx))
				{
					long dist;

					if ((vector2->vz - vector1->vz) >= 0)
					{
						if ((vector2->vz - vector1->vz) >= vector1->errory)
						{
							retValue = 0;
						}
						else
					...
					else if (dist = (vector1->vz - vector2->vz) < vector1->errory, dist != 0)
					{
						retValue = 1;
					}
				}
		...
		if (((POW2(vector2->vx - vector1->vx) + POW2(vector2->vy - vector1->vy) + POW2(vector2->vz - vector1->vz)) < POW2(vector1->errorx)) != 0)
		{
		return_1:
			retValue = 1;
		}
...
long EVENT_CompareRotationVectors(struct Rotation3d* rot1, struct Rotation3d* rot2, long trueValue)  // Matching - 100%
{
	MATRIX matrix1;
	MATRIX matrix2;
	long result;
	...
	case -1:
		...
		if ((PHYSOBS_CheckObjectAxisAlignment(&matrix1, &matrix2, &axis) - 3968) < 257U)
		{
			axis.x = 0;
			axis.y = 4096;
			axis.z = 0;
...
long EVENT_CompareListWithOperation(struct _PCodeStack* stack, struct ListObject* listObject, struct StackType* operand2, long operation)  // Matching - 100%
{
	long retValue;
	long d;
	struct StackType operand1;
	...
	temp = (char*)strchr((const char*)&objectName, 63);
	...
	while (instance = INSTANCE_FindWithName(areaID, (char*)&objectName, instance))
	{
		operand1.id = 2;

		operand1.data.listObject.eventInstance = (struct EventWildCardObject*)instance;
...
long EVENT_CompareSubListWithOperation(struct _PCodeStack* stack, struct SubListObject* subListObject, struct StackType* operand2, long operation)  // Matching - 100%
{
	long i;
	long retValue;
	long d;
	...
	for (i = 0; i < subListObject->numberOfInstances; i++)
	{
		...
		operand1.data.subListObject.instanceList = (struct _Instance**)subListObject->instanceList[i];
...
void EVENT_DoSubListAction(struct SubListObject* subListObject, struct StackType* operand2, struct _PCodeStack* stack, short* codeStream)  // Matching - 100%
{
	long i;
	long d;
	struct StackType operand1;
...
void EVENT_UpdatePuzzlePointers(struct EventPointers* events, long offset) // Matching - 100%
{
	long d;
	long d2;
	struct Event* curEvent;

	if (events != NULL)
	{
		for (d = 0; d < events->numPuzzles; d++)
		{
			events->eventInstances[d] = (struct Event*)OFFSET_DATA(events->eventInstances[d], offset);

			curEvent = events->eventInstances[d];

			curEvent->instanceList = (struct EventBasicObject**)OFFSET_DATA(curEvent->instanceList, offset);
			curEvent->conditionalList = (struct ScriptPCode**)OFFSET_DATA(curEvent->conditionalList, offset);
			curEvent->actionList = (struct ScriptPCode**)OFFSET_DATA(curEvent->actionList, offset);

			for (d2 = 0; d2 < curEvent->numInstances; d2++)
			{
				curEvent->instanceList[d2] = (struct EventBasicObject*)OFFSET_DATA(curEvent->instanceList[d2], offset);

				if (curEvent->instanceList[d2]->id == 2)
				{
					((struct EventWildCardObject*)(curEvent->instanceList[d2]))->objectName = (char*)OFFSET_DATA(((struct EventWildCardObject*)(curEvent->instanceList[d2]))->objectName, offset);
				}
			}
			for (d2 = 0; d2 < curEvent->numActions; d2++)
			{
				...
				{
					curEvent->conditionalList[d2] = (struct ScriptPCode*)OFFSET_DATA(curEvent->conditionalList[d2], offset);
					curEvent->conditionalList[d2]->data = (short*)OFFSET_DATA(curEvent->conditionalList[d2]->data, offset);
				}
				if (curEvent->actionList[d2] != NULL)
				{
					curEvent->actionList[d2] = (struct ScriptPCode*)OFFSET_DATA(curEvent->actionList[d2], offset);
...
void EVENT_SaveEventsFromLevel(long levelID, struct Level* level) // Matching - 100%
{
	...
	if (eventPointers != NULL)
	{
		for (i = 0; i < eventPointers->numPuzzles; i++)
		{
			...
			for (d = 0; d < 5; d++)
			{
				...
				unsigned short temp2;  // not from SYMDUMP
			...
			if (saveEvent != 0)
			{
				if (useBigSave != 0)
				{
					...
					savedEvent = (struct SavedEvent*)SAVE_GetSavedBlock(2, 0);
				...
				else
				{
					...
					savedEvent->areaID = (short)levelID;
					savedEvent->eventNumber = (char)eventInstance->eventNumber;

					for (d = 0; d < 5; d++)
					{
						savedEvent->eventVariables[d] = (char)eventInstance->eventVariables[d];
					}
				}
			}
...
void EVENT_FixPuzzlesForStream(struct _StreamUnit* stream) // Matching - 100%
{
	struct EventPointers* puzzle;
	struct EventBasicObject** basicEventObject;
	struct _Instance* instance;
	...
	struct EventEventVmObject* vmEventObject;  // not from SYMDUMP
	...
	if (puzzle != NULL)
	{
		...
		for (i = 0; i < puzzle->numPuzzles; i++)
		{
			...
			for (i2 = 0; i2 < puzzle->eventInstances[i]->numInstances; i2++)
			{
				...
				{
					struct _StreamUnit* newStream;

					wildcardEventObject = (struct EventWildCardObject*)basicEventObject[i2];

					newStream = STREAM_GetStreamUnitWithID(wildcardEventObject->unitID);

					if (newStream != NULL)
					{
						wildcardEventObject->objectName = (char*)newStream;
					}
				}
				else if (basicEventObject[i2]->id == 3)
				{
					struct _StreamUnit* newStream;
					struct EventEventObject* eventEventObject;
				...
				{
					struct EventTGroupObject* tgroupEventObject;
					struct _StreamUnit* newStream;
				...
				else if (basicEventObject[i2]->id == 6)
				{
					...
					newStream = STREAM_GetStreamUnitWithID(eventSignalObject->unitID);

					if (newStream != NULL)
					{
						eventSignalObject->mSignal = EVENT_ResolveObjectSignal(newStream, eventSignalObject->signalNum);
					}
				}
				else if (basicEventObject[i2]->id == 1)
				{
					...
					pointer = EVENT_ResolveObjectIntro(instanceEventObject);

					if (pointer != NULL)
					{
						instanceEventObject->data.intro = (struct Intro*)pointer;
					}
					else
					{
						struct _StreamUnit* stream2;

						stream2 = STREAM_GetStreamUnitWithID(instanceEventObject->unitID);

						if (stream2 != NULL)
						{
							pointer = EVENT_ResolveSFXMarker(stream2, instanceEventObject);

							if (pointer != NULL)
							{
								instanceEventObject->data.sfxMarker = (struct _SFXMkr*)pointer;

								instanceEventObject->flags |= 1;
							}
						}
					}
				}
				else if (basicEventObject[i2]->id == 7)
				{
					...
					vmEventObject = (struct EventEventVmObject*)basicEventObject[i2];

					newStream = STREAM_GetStreamUnitWithID(vmEventObject->unitID);

					if (newStream != NULL)
					{
						vmEventObject->vmObject = EVENT_FindVMObject(newStream, vmEventObject->objectName);
					}
				}
			}
		}
...
void EVENT_AddStreamToInstanceList(struct _StreamUnit* stream)  // Matching - 100%
{
	int d;
	int i;
	int i2;
		...
		for (i = 0; i < puzzle->numPuzzles; i++)
		{
			...
			for (i2 = 0; i2 < puzzle->eventInstances[i]->numInstances; i2++)
			{
				if (basicEventObject[i2]->id == 5)
				{
					...
					{
						wildcardEventObject->objectName = (char*)stream;
					}
				}
				else if (basicEventObject[i2]->id == 3)
				{
				...
				else if (basicEventObject[i2]->id == 6)
				{
					...
					{
						eventSignalObject->mSignal = EVENT_ResolveObjectSignal(stream, eventSignalObject->signalNum);
					}
				}
				else if (basicEventObject[i2]->id == 1)
				{
					...
					{
						pointer = EVENT_ResolveSFXMarker(stream, instanceEventObject);
				...
				else if (basicEventObject[i2]->id == 7)
				{
					...
					if (stream->StreamUnitID == vmEventObject->unitID)
					{
						vmEventObject->vmObject = (struct _VMObject*)EVENT_FindVMObject(stream, vmEventObject->objectName);
					}
				}
			}
		}
...
void EVENT_RelocateInstanceList(struct Level* oldLevel, struct Level* newLevel, long sizeOfLevel)  // Matching - 99.84%
{
	int d;
	int i;
	int i2;
	...
	offset = (long)newLevel - (long)oldLevel;
		...
		for (i = 0; i < puzzle->numPuzzles; i++)
		{
			...
			for (i2 = 0; i2 < puzzle->eventInstances[i]->numInstances; i2++)
			{
				if (basicEventObject[i2]->id == 3)
				{
					...
					if (IN_BOUNDS(eventEventObject->event, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						eventEventObject->event = (struct Event*)OFFSET_DATA(eventEventObject->event, offset);
					}
				}
				else if (basicEventObject[i2]->id == 4)
				{
					...
					if (IN_BOUNDS(tgroupEventObject->bspTree, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						tgroupEventObject->bspTree = (struct BSPTree*)OFFSET_DATA(tgroupEventObject->bspTree, offset);
					}
				}
				else if (basicEventObject[i2]->id == 6)
				{
					...
					if (IN_BOUNDS(eventSignalObject->mSignal, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						eventSignalObject->mSignal = (struct _MultiSignal*)OFFSET_DATA(eventSignalObject->mSignal, offset);
					}
				}
				else if (basicEventObject[i2]->id == 1)
				{
					...
					if (IN_BOUNDS(instanceEventObject->data.sfxMarker, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						instanceEventObject->data.sfxMarker = (struct _SFXMkr*)OFFSET_DATA(instanceEventObject->data.sfxMarker, offset);
					}
				}
				else if (basicEventObject[i2]->id == 7)
				{
					...
					if (IN_BOUNDS(vmEventObject->vmObject, oldLevel, (unsigned long)oldLevel + sizeOfLevel))
					{
						vmEventObject->vmObject = (struct _VMObject*)OFFSET_DATA(vmEventObject->vmObject, offset);
					}
				}
			}
		}
	...
	{
		int j;  // not from SYMDUMP
		struct EventTimer* et;  // not from SYMDUMP

		d = 0;

		et = eventTimerArray;

		j = (int)&numActiveEventTimers;

		do
		{
			if ((et[d].flags & 0x1) && (et[d].level == oldLevel))
			{
				et[d].event = (struct Event*)OFFSET_DATA(et[d].event, offset);
				et[d].actionScript = (struct ScriptPCode*)OFFSET_DATA(et[d].actionScript, offset);
				et[d].scriptPos = (short*)OFFSET_DATA(et[d].scriptPos, offset);
				et[d].level = newLevel;
			}
