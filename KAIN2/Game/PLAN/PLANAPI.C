...
void PLANAPI_DoTimingCalcsAndDrawing(long startTime, struct PlanningNode* planningPool) // Matching - 98.10%
{
	int i;
	long minTime;
	long maxTime;
...
int PLANAPI_PassThroughHit(struct PlanningNode* node1, struct PlanningNode* node2) // Matching - 99.81%
{
	int src1;
	int src2;
	int res;
	...
	{
		src1 = node1->nodeType & 7;
		src2 = node2->nodeType & 7;
		...
		res = (src1 << 8) | src2;
		if (res == 0x104 || res == 0x304 || res == 0x204)
		{
			return 1;
		}
	}
