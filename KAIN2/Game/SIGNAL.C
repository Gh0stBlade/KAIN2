...
long SIGNAL_HandleSetCameraTilt(struct _Instance* instance, struct Signal* signal) // Matching - 100%
{
	long tilt;
...
struct _MultiSignal* SIGNAL_RelocateSignal(struct _MultiSignal* multiSignal, long offset) // Matching - 100%
{
	int i;
	...
	return (struct _MultiSignal*)&signal->data.callSignal;
}


struct _MultiSignal* SIGNAL_FindSignal(struct Level* level, long id) // Matching - 100%
{
	int i;
	struct Signal* signal;
	...
	while (msignal < level->SignalListEnd)
	{
		...
		msignal = (struct _MultiSignal*)&signal->data.callSignal;
	}
