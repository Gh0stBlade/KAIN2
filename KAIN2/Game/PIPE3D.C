...
void PIPE3D_TransformSplitInstanceVertices(struct _MVertex* vertexList, struct _PVertex* pvertex, struct _Model* model, MATRIX* wpTransform, MATRIX* matrixPool, struct _Mirror* mirror) // Matching - 97.85%
{
	MATRIX* spTransform;
	struct _Vector* vector;
	long i;
	...
	vector = (struct _Vector*)getScratchAddr(8);

	spTransform = (MATRIX*)getScratchAddr(0);

	for (i = 0, segment = model->segmentList; i < model->numSegments; segment++, i++)
	{
		if (model->segmentList[i].lastVertex != -1)
		{
			struct _MVertex* firstVertex;
			struct _MVertex* lastVertex;
			struct _MVertex* modelVertex;
