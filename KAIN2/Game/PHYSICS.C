...
int PhysicsCheckLinkedMove(struct _Instance* instance, int Data, short Mode)  // Matching - 100%
{
	...
	if (instance->matrix != NULL)
	{
		...
		ptr = (struct evPhysicsLinkedMoveData*)Data;
		...
		{
			struct _G2EulerAngles_Type newRot;
			VECTOR newRelPos;
			long oldPosX;
			long oldPosY;
			long oldPosZ;
			...
			G2EulerAngles_FromMatrix(&newRot, (struct _G2Matrix_Type*)&work, 21);
		...
		if ((Mode & 4))
		{
			PhysicsDefaultLinkedMoveResponse(instance, ptr, (((unsigned)(Mode & 0xFFFF) >> 3) ^ 1) & 1);
		}
...
void PhysicsForceSetWater(struct _Instance* instance, int* Time, int Depth, int rate, int maxAmplitude) // Matching - 100%
{
	int Amplitude;
	...
	*Time = *Time + ((rate << 0xC) / gameTrackerX.timeMult);
	if (*Time >= 4097)
	{
		*Time = *Time - 4096;
	}
	if (Depth < 0)
	{
		instance->position.z = (instance->position.z + (Amplitude * rcos(*Time)) / 4096);
	}
}
...
int PhysicsCheckDropHeight(struct _Instance* instance, int Data, int Mode) // Matching - 99.88%
{
	struct evPhysicsDropHeightData* data;
	struct Level* level;
	SVECTOR newPos;
	SVECTOR oldPos;
	int rc;
	...
	data = (struct evPhysicsDropHeightData*)Data;
	...
	}
	{
		...
		if (lowZ < oldPos.vz)
		{
			...
			if (CInfo.type != 0)
			{
				if (CInfo.wNormal.vz >= 3201)
				{
					if (Mode == 1)
					{
						if (CInfo.type == 3)
						{
							level = (struct Level*)CInfo.inst;

							data->bsp = &level->terrain->BSPTreeArray[CInfo.segment];
							data->tface = (struct _TFace*)CInfo.prim;
						}
						else
						{
							data->bsp = NULL;
...
void PhysicsSetVelFromRot(struct _Instance* instance, struct _Rotation* rot, long magnitude) // Matching - 100%
{
	MATRIX mat;
	SVECTOR flatPt;
	SVECTOR newPt;
...
void PHYSICS_SetVAndAFromRot(struct _Instance* instance, struct _Rotation* rot, long v, long a) // Matching - 100%
{
	SVECTOR flatPt;
	MATRIX mat;
...
int PHYSICS_CheckDontGrabEdge(struct _PCollideInfo* CInfo)  // Matching - 100%
{
	int rc;
	struct _TFace* tface;
	struct BSPTree* bsp;
	...
	if (CInfo->type == 3)
	{
		...
		if (tface->textoff != 0xFFFF)
		{
			hface = (struct _HFace*)((char*)(((struct _Terrain*)CInfo->inst->node.prev)->StartTextureList) + tface->textoff);
			if ((hface->n0 & (unsigned int)0x80) > (unsigned int)rc)
			{
				rc = 1;
			}
		}
		bsp = ((struct _Terrain*)CInfo->inst->node.prev)->BSPTreeArray + CInfo->segment;
		if (bsp->flags & 0x8000)
		{
			rc = 1;
		}
	...
	{
		hface = (struct _HFace*)CInfo->prim;
		if (hface->attr & 0x80)
		{
			rc = 1;
		}
	...
	{
		if (!(INSTANCE_Query(CInfo->inst, 1) & 0x20) || (INSTANCE_Query(CInfo->inst, 3) & 1))
		{
			rc = 0;
		}
		else
