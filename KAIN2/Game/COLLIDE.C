...
struct _SVector collide_localPoint; // offset 0x800D2708
MATRIX* collide_lwTransform; // offset 0x800D2710
...
long COLLIDE_WithinYZBounds(struct _SVector* point, struct _HBox* hbox)  // Matching - 100%
{
	int temp;  // not from SYMDUMP
...
long COLLIDE_WithinXZBounds(struct _SVector* point, struct _HBox* hbox)  // Matching - 100%
{
	int temp;  // not from SYMDUMP
...
void COLLIDE_PointAndInstance(struct _PCollideInfo* pcollideInfo, struct _Instance* instance) // Matching - 100%
{
	MATRIX* swTransform;
	MATRIX* wsTransform;
	MATRIX* swNormMat;
	struct _Vector* oldPosVec;
	struct _Vector* lNormal;
	struct _Vector* wNormal;
	struct _Vector* dv;
	...
	oldPosVec = (struct _Vector*)getScratchAddr(76);

	lNormal = (struct _Vector*)getScratchAddr(84);

	wNormal = (struct _Vector*)getScratchAddr(88);

	dv = (struct _Vector*)getScratchAddr(96);

	newPosVec = (struct _Vector*)getScratchAddr(100);

	oldPos = (struct _SVector*)getScratchAddr(104);

	newPos = (struct _SVector*)getScratchAddr(106);

	point = (struct _SVector*)getScratchAddr(108);
	...
	for (i = hmodel->numHPrims, hprim = hmodel->hPrimList; i != 0; i--, hprim++)
	{
		if (((hprim->hpFlags & 0x1)) && ((hprim->hpFlags & 0x8)))
		{
			...
			*(tmm*)(swNormMat->t) = *(tmm*)(swTransform->t);
			...
			case 5:
			{
				struct _SVector hbNormal;
				struct _SVector point0;
				struct _SVector point1;
				...
				if ((hbox->flags & 0x2000))
				{
					...
					if (COLLIDE_IntersectLineAndBox(&point0, &hbNormal, &point1, &normal1, newPos, oldPos, hbox) != 0)
					{
						...
						short _y1;
						short _z1;
						struct _SVector* _v1;  // not from SYMDUMP
...
long COLLIDE_ClosestPointInBoxToPoint(struct _Position* boxPoint, struct _HBox* hbox, struct _SVector* point) // Matching - 100%
{
	long inside;
...
long COLLIDE_SphereAndPoint(struct _Sphere* sphere, struct _SVector* point, struct _SVector* normal) // Matching - 99.17%
{
	long len;
	struct _Vector* line;
	...
	line = (struct _Vector*)getScratchAddr(0);

	{
		...
		_x0 = sphere->position.x;
		_y0 = sphere->position.y;
		_z0 = sphere->position.z;
...
void COLLIDE_Instance1SpheresToInstance2(struct _Instance* instance1, struct _Instance* instance2, long sphereToSphere) // Matching - 97.84%
{
	MATRIX* wsTransform2;
	MATRIX* oldWSTransform2;
	struct _Position* spherePos;
	struct _Vector* line;
	struct _Vector* offset;
	struct _Vector* tempVec;
	struct _Vector* sSpherePos1;
	struct _Vector* oldSSpherePos1;
	struct _SVector* svec;
	struct _Sphere* sSphere1;
	struct _Sphere* oldSSphere1;
	struct _Sphere* wSphere1;
	struct _Sphere* wSphere2;
	struct _Sphere* oldWSphere1;
	struct _Sphere* oldWSphere2;
	...
	struct _HSphere* hsphere1;
	struct _HSphere* hsphere2;
	struct _HFace* hface;
	long flag;
	long edge;
	struct _HModel* hmodel1;
	struct _HModel* hmodel2;
	struct _HPrim* hprim1;
	struct _HPrim* hprim2;
	int i;
	...
	spherePos = (struct _Position*)getScratchAddr(74);

	line = (struct _Vector*)getScratchAddr(82);

	offset = (struct _Vector*)getScratchAddr(86);

	tempVec = (struct _Vector*)getScratchAddr(90);

	sSpherePos1 = (struct _Vector*)getScratchAddr(98);

	oldSSpherePos1 = (struct _Vector*)getScratchAddr(102);

	svec = (struct _SVector*)getScratchAddr(106);

	sSphere1 = (struct _Sphere*)getScratchAddr(108);

	oldSSphere1 = (struct _Sphere*)getScratchAddr(111);

	wSphere1 = (struct _Sphere*)getScratchAddr(120);

	wSphere2 = (struct _Sphere*)getScratchAddr(123);

	oldWSphere1 = (struct _Sphere*)getScratchAddr(126);

	oldWSphere2 = (struct _Sphere*)getScratchAddr(129);

	collideInfo = (struct _CollideInfo*)getScratchAddr(132);

	hfaceInfo = (struct _HFaceInfo*)getScratchAddr(144);

	hmodel1 = &instance1->hModelList[instance1->currentModel];

	if (instance1->hModelList != NULL)
	{
		if (instance2->hModelList != NULL)
		{
			hmodel2 = &instance2->hModelList[instance2->currentModel];

			if (hmodel2->numHPrims != 0)
			{
				for (hprim1 = hmodel1->hPrimList, i = hmodel1->numHPrims; i != 0; i--, hprim1++)
				{
					if (((hprim1->hpFlags & 0x1)) && ((hprim1->withFlags & 0x4)) && (hprim1->type == 1))
					{
						...
						wSphere1->position.x = (short)tempVec->x;
						wSphere1->position.y = (short)tempVec->y;
						wSphere1->position.z = (short)tempVec->z;
						...
						oldWSphere1->position.x = (short)tempVec->x;
						oldWSphere1->position.y = (short)tempVec->y;
						oldWSphere1->position.z = (short)tempVec->z;

						for (hprim2 = hmodel2->hPrimList, i = hmodel2->numHPrims; i != 0; i--, hprim2++)
						{
							if ((hprim2->hpFlags & 0x1) && (hprim2->hpFlags & 0x4))
							{
								...
								if ((hprim2->type == 1) && ((hprim1->withFlags & 0x20)) && (hprim2->withFlags & 0x20))
								{
									...
									wSphere2->position.x = (short)tempVec->x;
									wSphere2->position.y = (short)tempVec->y;
									wSphere2->position.z = (short)tempVec->z;
									...
									oldWSphere2->position.x = (short)tempVec->x;
									oldWSphere2->position.y = (short)tempVec->y;
									oldWSphere2->position.z = (short)tempVec->z;
									...
									if (((line->x * line->x) + (line->y * line->y) + (line->z * line->z)) < ((hsphere1->radius + hsphere2->radius) * (hsphere1->radius + hsphere2->radius)))
									{
										...
										if (len != 0)
										{
											...
											line->x = line->x * ((hsphere1->radius + hsphere2->radius) - (len >> 5));
											line->y = line->y * ((hsphere1->radius + hsphere2->radius) - (len >> 5));
											line->z = line->z * ((hsphere1->radius + hsphere2->radius) - (len >> 5));

											collideInfo->type0 = 1;
											collideInfo->type1 = 1;

											collideInfo->offset.x = (short)((line->x << 5) / len);
											collideInfo->offset.y = (short)((line->y << 5) / len);
											collideInfo->offset.z = (short)((line->z << 5) / len);
								...
								{
									if ((hprim2->type == 2) || (hprim2->type == 5))
									{
										struct _Model* model2;
										...
										sSphere1->position.x = (short)sSpherePos1->x;
										sSphere1->position.y = (short)sSpherePos1->y;
										sSphere1->position.z = (short)sSpherePos1->z;
										...
										oldSSphere1->position.x = (short)oldSSpherePos1->x;
										oldSSphere1->position.y = (short)oldSSpherePos1->y;
										oldSSphere1->position.z = (short)oldSSpherePos1->z;
										...
										if ((hprim2->type == 2) && ((hprim1->withFlags & 0x40)) && ((hprim2->withFlags & 0x20)))
										{
											...
											hfaceInfo->vertex0 = (struct _HVertex*)&model2->vertexList[hface->v0];
											hfaceInfo->vertex1 = (struct _HVertex*)&model2->vertexList[hface->v1];
											hfaceInfo->vertex2 = (struct _HVertex*)&model2->vertexList[hface->v2];

											COLLIDE_GetNormal(hface->normal, (short*)model2->normalList, &hfaceInfo->normal);

											if (COLLIDE_SphereAndHFace(sSphere1, (struct _Position*)oldSSphere1, hfaceInfo, (struct _SVector*)&collideInfo->point1, &edge) != 0)
											{
												...
												if (collideFunc != NULL)
												{
													...
													collideInfo->offset.x = (short)-offset->x;
													collideInfo->offset.y = (short)-offset->y;
													collideInfo->offset.z = (short)-offset->z;
										...
										{
											if ((hprim2->type == 5) && ((hprim1->withFlags & 0x10)) && ((hprim2->withFlags & 0x20)))
											{
												struct _HBox* hbox;
												...
												if (COLLIDE_SphereAndHBox(hbox, sSphere1, (struct _Position*)oldSSphere1, (struct _SVector*)&collideInfo->point0) != 0)
												{
													svec->x = (sSphere1->position.x - spherePos->x);
													svec->y = (sSphere1->position.y - spherePos->y);
													svec->z = (sSphere1->position.z - spherePos->z);
													...
													if (collideFunc != NULL)
													{
														collideInfo->type0 = hprim2->type;
														collideInfo->flags = 0x2;
														collideInfo->type1 = 1;
														collideInfo->prim0 = hbox;
														collideInfo->prim1 = hsphere1;
														collideInfo->inst0 = instance2;
														collideInfo->inst1 = instance1;
													...
													if (collideFunc != NULL)
													{
														collideInfo->flags = 0;
														collideInfo->type0 = 1;
														collideInfo->type1 = hprim2->type;
														collideInfo->prim0 = hsphere1;
														collideInfo->prim1 = hbox;
														collideInfo->inst0 = instance1;
														collideInfo->inst1 = instance2;
