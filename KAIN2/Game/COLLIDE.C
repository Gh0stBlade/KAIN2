...
void COLLIDE_MakeNormal(struct _Terrain* terrain, struct _TFace* tface, struct _SVector* normal)
{
	...
	_Vector* n;
	int iVar4;
	vertex0 = (_SVector*)&terrain->vertexList[tface->face.v1].vertex;
	vertex1 = (_SVector*)&terrain->vertexList[tface->face.v0].vertex;
	{
		int _x0;
		int _y0;
		int _z0;
		...
		_z1 = vertex1->z;
		{
			a = (_Vector*)getScratchAddr(0);
			a->x = _x0 - _x1;
			a->y = _y0 - _y1;
			a->z = _z0 - _z1;
		}
	}
	vertex2 = (_SVector*)&terrain->vertexList[tface->face.v2].vertex;
	{
		int _x0;
		int _y0;
		int _z0;
	...
	{
		n = (_Vector*)getScratchAddr(8);
		n->y = (int)(short)-(short)(a->x * b->z - a->z * b->x >> 0xc);
		len = n->y;
		if (n->y < 0)
		{
			len = -n->y;
		}
		n->x = (a->y * b->z - a->z * b->y) * 0x10 >> 0x10;
		iVar4 = n->x;
		if (n->x < 0)
		{
			iVar4 = -n->x;
		}
		n->z = (a->x * b->y - a->y * b->x) * 0x10 >> 0x10;
		if (len < iVar4)
		{
			len = iVar4;
		}
		...
		if (len != 0)
		{
			normal->x = (short)((n->x << 0xc) / len);
			normal->y = (short)((n->y << 0xc) / len);
			normal->z = (short)((n->z << 0xc) / len);
		}
	}
#elif defined(PC_VERSION)
