...
long PIPE3D_TransformAnimatedInstanceVertices_S(struct _VertexPool* vertexPool, struct _PVertex* poolVertex, struct _Model* model, MATRIX* wcTransform, MATRIX* matrixPool, MATRIX* lm, CVECTOR* vertexColor, CVECTOR* perVertexColor)
{
	...
	{
		//t1 = matrixPool
		//t8 = model->segmentList->firstVertex//AS INT!
		//t9 = vertexPool
		//s1 = vertexColor
		//s2 = &gNormalList
		//s6 = perVertexColor
		...
		if (vertexColor != NULL)
		{
			...
			if (perVertexColor == NULL)
			{
				...
				//loc_80027244
				//a0 =  model->segmentList->
				//v0 = model->segment->lastVertex

				//t7 = v0 << 3
				if (model->segmentList->lastVertex + 1 != 0)
				{
					struct _PVertex* t7 = &vertexPool->vertex[model->segmentList->lastVertex];
					struct _PVertex* a1 = &vertexPool->vertex[model->segmentList->firstVertex];

					gte_SetRotMatrix(&scratchAddr[13]);

					gte_ldclmv(&matrixPool->m[0][0]);

					gte_rtir();

					gte_stclmv(&scratchAddr[18]);

					gte_ldclmv(&matrixPool->m[0][1]);

					gte_rtir();

					gte_stclmv(&scratchAddr[19]);

					gte_ldclmv(&matrixPool->m[0][2]);

					gte_rtir();

					gte_stclmv(&scratchAddr[20]);

					gte_SetLightMatrix(&scratchAddr[18]);

					//v0 = &scratchAddr[0]
					//v1 = &scratchAddr[8]
					gte_SetRotMatrix(&scratchAddr[0]);

					gte_ldclmv(&matrixPool->m[0][0]);

					gte_rtir();

					gte_stclmv(&scratchAddr[8]);

					gte_ldclmv(&matrixPool->m[0][1]);

					gte_rtir();

					gte_stclmv(&scratchAddr[9]);

					gte_ldclmv(&matrixPool->m[0][2]);

					gte_rtir();

					gte_stclmv(&scratchAddr[10]);

					gte_SetTransMatrix(&scratchAddr[5]);

					gte_ldv0(&matrixPool->t[0]);

					gte_rt();

					gte_stlvl(&scratchAddr[18]);

					gte_SetTransMatrix(&scratchAddr[3]);

					gte_SetLightMatrix(&scratchAddr[8]);

					//struct TwoVector* a1 = (struct TwoVector*)&vertexPool->vertex[model->segmentList->firstVertex];

					if (&vertexPool->vertex[model->segmentList->lastVertex] >= &vertexPool->vertex[model->segmentList->firstVertex + 2])
					{
						//loc_80027500
						{
							gte_SetRotMatrix(&vertexPool->vertex[model->segmentList->firstVertex]);

							gte_ldv3c(vertexPool->vertex[model->segmentList->firstVertex]);

							gte_rtpt();

							//s3 = gNormalList[((MATRIX*)&vertexPool->vertex[model->segmentList->lastVertex])->m[0][1]]
							//s4 = gNormalList[((MATRIX*)&vertexPool->vertex[model->segmentList->lastVertex])->m[1][0]]
							//s5 = gNormalList[((MATRIX*)&vertexPool->vertex[model->segmentList->lastVertex])->m[1][2]]

							gte_stsxy3(((VECTOR*)poolVertex)->vx, ((VECTOR*)poolVertex)->vy, ((VECTOR*)poolVertex)->vz);
							gte_stsz3((((VECTOR*)poolVertex) + 1)->vx, (((VECTOR*)poolVertex) + 1)->vy, (((VECTOR*)poolVertex) + 1)->vz);
							...
							//struct TwoVector* t4 = (struct TwoVector*)&a1->.y;
							...
							//This is totally messed, looks like something is incorrectly coded in original game i.e struct size mis-matches.
						}
					}
