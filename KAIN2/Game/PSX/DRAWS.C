...
long DRAW_DisplayTFace_S(unsigned long** pNextPrim, unsigned long** drawot, int s2, unsigned long* t4)
{ 
	int t8 = FLAG;
...
subdiv_begin:
	...
	not_liquid_26:
		...
		if (DRAW_DisplayTFace_S(&pNextPrim, drawot, s2, t4) <= 0)
		{
			gte_ldv0(&t4[20]);
			gte_ldv1(&t4[18]);
			gte_ldv2(&t4[14]);
			...
			if (DRAW_DisplayTFace_S(&pNextPrim, drawot, s2, t4) <= 0)
			{
				...
				if(DRAW_DisplayTFace_S(&pNextPrim, drawot, s2, t4) <= 0)
				{
					...
					if (DRAW_DisplayTFace_S(&pNextPrim, drawot, s2, t4) <= 0)
					{
						...
						DRAW_DisplayTFace_S(&pNextPrim, drawot, s2, t4);
