...
void DRAW_DisplaySubdivPolytope_S(_PolyTope* polytope, struct _Terrain* terrain) 
...
{ 
	...
	start = (_PolyTope*)&polytopeList->polytope[0];
	end = (_PolyTope*)&polytopeList->polytope[polytopeList->numPolytopes];
	...
	while ((char*)start < (char*)end)
	{
		if ((start->flags & 0x2) && (char*)&primPool->nextPrim[768] < (char*)primPool->lastPrim)
		{
			...
			if (NULL != gameTrackerX.gameData.asmData.lightInstances[0].lightInstance)
			{
				...
				x = (lm->t[0] - globalOffset->x) - start->x;
				y = (lm->t[1] - globalOffset->y) - start->y;
				z = (lm->t[2] - globalOffset->z) - start->z;
				...
				if (x < gameTrackerX.gameData.asmData.lightInstances[0].radius + start->radius)
				{
