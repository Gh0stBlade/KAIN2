...
void DRAW_DisplaySubdivPolytope_S(struct _BSPNode* polytope, struct _Terrain* terrain)
...
{ 
	...
	start = (struct _BSPNode*)polytopeList->polytope[0];
	end = (struct _BSPNode*)polytopeList->polytope[polytopeList->numPolytopes];
	...
	{
		if ((start->flags & 0x2) && (char*)&primPool->nextPrim[768] < (char*)primPool->lastPrim)
		{
			...
			if (NULL != gameTrackerX.gameData.asmData.lightInstances[0].lightInstance)
			{
				...
				x = (lm->t[0] - globalOffset->x) - start->sphere.position.x;
				y = (lm->t[1] - globalOffset->y) - start->sphere.position.y;
				z = (lm->t[2] - globalOffset->z) - start->sphere.position.z;
				...
				if (x < gameTrackerX.gameData.asmData.lightInstances[0].radius + start->sphere.radius)
				{
					getScratchAddr(0)[6] = y;
				}
			}

			DRAW_DisplaySubdivPolytope_S(start, terrain);
		}
