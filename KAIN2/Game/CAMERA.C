...
void CAMERA_CenterCamera(struct Camera* camera)
{
	...
	if ((camera->instance_mode & 0x2000000))
	{
		tmp1 = CAMERA_AngleDifference(camera->focusRotation.z, camera->focusInstance->rotation.z + 1024);
		
		tmp2 = CAMERA_AngleDifference(camera->focusRotation.z, camera->focusInstance->rotation.z - 1024);
	...
	if (!(camera->flags & 0x10000) && camera->mode != 6 && !(camera->instance_mode & 0x3002043))
	{
		...
		if (speedxy >= 0x17)
		{
			...
			if (angle - 401 < 1199)
			{
				...
				if (CAMERA_AngleDifference(lead_target, camera->lead_angle) >= 0x51)
				{
					lead_smooth = 12;
				}
				else
