...
void CAMERA_CreateNewFocuspoint(struct Camera* camera) // Matching - 100%
{
	struct _SVector sv;
...
void CAMERA_Adjust_roll(long roll_degrees, int frames) // Matching - 100%
{
	long angle;  // not from SYMDUMP

	angle = roll_degrees & 0xFFF;
	if (frames == 0)
	{
		current_roll_amount = angle * 4096;
		roll_target = current_roll_amount;
		roll_inc = 0;
	}
	else
	{
		roll_inc = CAMERA_SignedAngleDifference((short)angle, (short)(current_roll_amount / 4096)) * 4096 / frames;
		roll_target = angle * 4096;
	}
}
...
void CAMERA_SetShake(struct Camera* camera, long shake, long scale) // Matching - 100%
{
	int shock;
	int duration;
	long cameraShake; // not from SYMDUMP
	long cameraShakeScale; // not from SYMDUMP
	...
	else
	{
		...
		if (cameraShake < shake << 12)
		{
			cameraShake = shake << 12;
		}
		camera->shake = cameraShake;
		cameraShakeScale = camera->shakeScale;
		if (cameraShakeScale < scale)
		{
			cameraShakeScale = scale;
		}
		camera->shakeScale = (short)cameraShakeScale;
	}
...
void CAMERA_LookProcess(struct Camera* camera) // Matching - 99.90%
{
	...
	if ((camera->instance_mode & 0x20000) && (distance < 0x226))
	{
		distance = 0x226;
	}
...
void CAMERA_CinematicProcess(struct Camera* camera) // Matching - 100%
{
	struct _SVector* camPos;
	struct _SVector* camTarget;
	...
	{
		CriticalDampPosition(4, &camera->core.position, &camera->targetPos, &camera->positionVel, &camera->positionAccl, -camera->smooth);

		if (camera->data.Cinematic.targetSpline != NULL)
		{
			CriticalDampPosition(4, &camera->focusPoint, &camera->targetFocusPoint, &camera->focusPointVel, &camera->focusPointAccl, -camera->smooth);

			CAMERA_SetTarget(camera, &camera->focusPoint);

			if ((camera->core.position.x - camera->targetPos.x) >= 0)
			{
				if ((camera->core.position.x - camera->targetPos.x) < 9)
				{
					goto block_1;
				}
			}
			...
			{
				if ((camera->targetPos.x - camera->core.position.x) < 9)
				{
				block_1:
					if ((camera->core.position.y - camera->targetPos.y) >= 0)
					{
						if ((camera->core.position.y - camera->targetPos.y) < 9)
						{
							goto block_2;
						}
					}
					...
					{
						if ((camera->targetPos.y - camera->core.position.y) < 9)
						{
						block_2:
							if ((camera->core.position.z - camera->targetPos.z) >= 0)
							{
								if ((camera->core.position.z - camera->targetPos.z) < 9)
								{
									goto block_3;
								}
							}
							...
							{
								if ((camera->targetPos.z - camera->core.position.z) < 9)
								{
								block_3:
									if ((camera->focusPoint.x - camera->targetFocusPoint.x) >= 0)
									{
										if ((camera->focusPoint.x - camera->targetFocusPoint.x) < 9)
										{
											goto block_4;
										}
									}
									...
									{
										if ((camera->targetFocusPoint.x - camera->focusPoint.x) < 9)
										{
										block_4:
											if ((camera->focusPoint.y - camera->targetFocusPoint.y) >= 0)
											{
												if ((camera->focusPoint.y - camera->targetFocusPoint.y) < 9)
												{
													goto block_5;
												}
											}
											...
											{
												if ((camera->targetFocusPoint.y - camera->focusPoint.y) < 9)
												{
												block_5:
													if ((camera->focusPoint.z - camera->targetFocusPoint.z) >= 0)
													{
														if ((camera->focusPoint.z - camera->targetFocusPoint.z) < 9)
														{
															goto block_6;
														}
													}
													...
													{
														if ((camera->targetFocusPoint.z - camera->focusPoint.z) < 9)
														{
														block_6:
															camera->posState = 0;
														}
