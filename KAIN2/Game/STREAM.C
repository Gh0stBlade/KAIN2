...
void STREAM_FinishLoad(struct _StreamUnit *streamUnit)
{
	...
	if (level->startUnitLoadedSignal != NULL)
	{
		...
		SIGNAL_HandleSignal(gameTrackerX.playerInstance, (struct Signal*)level->startUnitLoadedSignal, 8);
...
void RelocatePlanPool(struct PlanningNode *planPool, _SVector *offset)
...
void MORPH_ToggleMorph()
{
	...
	SOUND_PlaneShift(gameTrackerX.gameData.asmData.MorphType < 1);
...
{
	int tempX = (((v->vx >> 12) - (theCamera.core.position.x << 4)) * theCamera.core.vvNormalWorVecMat[k].m[j][0]);
	int tempY = (((v->vy >> 12) - (theCamera.core.position.y << 4)) * theCamera.core.vvNormalWorVecMat[k].m[j][1]);
	int tempZ = (((v->vz >> 12) - (theCamera.core.position.z << 4)) * theCamera.core.vvNormalWorVecMat[k].m[j][2]);
