...
int WARPGATE_IsObjectOnWarpSide(struct _Instance* instance)  // Matching - 99.60%
{
	...
	if (wgloadInfo.warpFaceInstance != NULL)
	{
		side = ~(wgloadInfo.warpFaceInstance->position.y - theCamera.core.position.y);
		temp = side < 0;
		temp2 = temp;
		if ((wgloadInfo.warpFaceInstance->position.y - instance->position.y) < 0)
		{
			if (temp == 1)
			{
				return 1;
