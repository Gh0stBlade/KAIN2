...
int GetCollisionType(struct _Instance *instance)
{
	...
	struct _CollideInfo* v3; // $a0
	int result; // $v0
	struct _Instance* inst; // $v1
	int v6; // $a0

	collideInfo = (struct _CollideInfo*)instance->collideInfo;
	if (*(int*)((char*)collideInfo->prim0 + 4) == 9)
	{
		v3 = (struct _CollideInfo*)instance->collideInfo;
		if (collideInfo->type1 == 3)
			COLLIDE_SetBSPTreeFlag(v3, 2048);
		else
	...
	LABEL_18:
		...
		v6 = (unsigned char)collideInfo->type1;
		if (v6 == 5)
			return (ControlFlag & 0x8000000) != 0;
	...
	if (GetCollisionType(instance) != 1)
	{
		if (collideInfo->type1 != 1)
		{
			if ((collideInfo->type0 == 4 || collideInfo->type0 == 1)
				&& (collideInfo->type1 == 3 && ((((char*)collideInfo->prim1)[6]) & 8) == 0
					|| collideInfo->type1 == 5 && (*(unsigned short*)collideInfo->prim1 & 0x2000)
					|| collideInfo->type1 == 2))
			{
				offset = (SVECTOR*)&((struct _CollideInfo*)instance->collideInfo)->offset;

				if ((*(unsigned int*)&((struct _CollideInfo*)instance->collideInfo)->flags & 0xA) != 0)
				{
		...
		else
		{
			...
			if ((*(unsigned int*)collideInfo->prim1 & 0x2000) != 0)
			{
				offset = (SVECTOR*)&((struct _CollideInfo*)instance->collideInfo)->offset;
				offset->vz = 0;
				instance->offset.x += offset->vx;
