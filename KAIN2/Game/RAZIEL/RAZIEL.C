...
void RazielPost(struct _Instance* instance, unsigned long Message, unsigned long Data)  // Matching - 95.72%
{
	int i; // $s0
	struct PlayerSaveData* data;
	struct _G2AnimSection_Type* animSection;
	...
	case 0x100010:
		...
		{
			// @fixme this bit gets optimized out in decompiled MIPS and causes break to jump to case 0x100012
	...
	case 0x4000E:
		...
		else
		{
			...
			// @fixme here i should be stored in $s2 register but decompiled MIPS uses $s0 instead
			for (i = 0; i < 3; i++)
			{
				PurgeMessageQueue(&Raziel.State.SectionList[i].Event);
				PurgeMessageQueue(&Raziel.State.SectionList[i].Defer);
