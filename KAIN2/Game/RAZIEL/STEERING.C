...
void SteerTurn(struct _Instance* instance, int rc)
{
	...
	if (rc != 0 || Raziel.Bearing != 0)
	{
		...
		{
			rot = instance->rotation.z;
			
			rot -= Raziel.LastBearing;
		...
		AngleMoveToward(&instance->rotation.z, Raziel.LastBearing, (Raziel.steeringVelocity * gameTrackerX.timeMult) >> 12);
	}
	else
	{
...
void SteerMove(struct _Instance* instance, int rc)
{
	if (rc != 0 || Raziel.Bearing != 0)
	{
		Raziel.steeringVelocity = ABS(Raziel.Bearing);

		if (Raziel.steeringVelocity >= 769)
		{
			if (Raziel.steeringVelocity < 0)
			{
				Raziel.steeringVelocity = ((Raziel.steeringVelocity + 3) >> 2) + 112;
			}
			else
			{
				Raziel.steeringVelocity = ((Raziel.steeringVelocity) >> 2) + 112;
			}
		}
		else
		{
			Raziel.steeringVelocity = 112;
		}
