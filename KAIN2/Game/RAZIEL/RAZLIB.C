...
struct __razController controllerList[24]; // offset 0x800d1c64
...
void razSelectMotionAnim(struct __CharacterState* In, int CurrentSection, int Frames, int* Anim)
{
	...
	if (CurrentSection == 0)
	{
		if (!(ControlFlag & 0x20000000))
		{
			...
			Vec.y = ((Vec.y * G2Timer_GetFrameTime()) * adjustment) / 409600;
		...
		if (CurrentSection != 0)
		{
			...
			{
				animSectionA = &In->CharacterInstance->anim.section[CurrentSection];
				animSectionB = &In->CharacterInstance->anim.section[CurrentSection + 1];
