...
void StateHandlerStumble(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 100%
{
    struct __Event* Ptr;
        ...
        if (Ptr != NULL)
        {
            ...
            case 0x40003:
                if (CurrentSection == 0)
                {
                    ...
                    data = (struct evActionPlayHostAnimationData*)Ptr->Data;
...
void StateHandlerThrow2(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 100%
{
    struct __Event* Ptr;
    int Anim;
    ...
    while (Ptr = PeekMessageQueue(&In->SectionList[CurrentSection].Event), Ptr != NULL)
    {
        switch (Ptr->ID)
        {
        case 0x100001:
            if (CurrentSection == 0)
            {
                ...
                if (razGetHeldWeapon() != NULL)
                {
                    ...
                    case 0x1000:
                        ...
                        razSetupSoundRamp(In->CharacterInstance, (struct _SoundRamp*)&Raziel.soundHandle, 12, -100, 0, 50, 85, 184320, 10000);
        ...
        case 0x8000000:
            ...
            else if (Anim == Raziel.throwData->throwAnim)
            {
                if (CurrentSection == 1)
                {
        ...
        case 0x8000004:
        {
            ...
            if (weaponInst != NULL)
            {
                ...
                {
                    INSTANCE_Post(weaponInst, 0x800010, SetObjectThrowData(NULL, NULL, NULL, spin_type, Raziel.throwData->velocity, 0, 0, 0));
                }
