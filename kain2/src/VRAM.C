...
struct _BlockVramEntry* VRAM_CheckVramSlot(short* x, short* y, short w, short h, int type, int startY)
{
	...
	if (vblock != NULL)
	{
		do
		{
			...
			{
				if ((vblock->x & 0x3F) && (64 - (vblock->x & 0x3F)) != 0)
				{
...
#define ABS(x) x < 0 ? -x : x
			...
			if (ABS(vblock->y * (hldh - h)) < ABS(((offsetright - vblock->y) * h) - (offsetright * (hldh - h))))
			{
				VRAM_InsertFreeVram(hldx, hldy, offset - w, hldh, 1);
