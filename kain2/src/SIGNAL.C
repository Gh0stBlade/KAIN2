	...
	if (numSignals)
	{
		while (1)
		{
				...
				&& ((gameTrackerX.gameFlags & 0x40) == 0 || signal->id >= 0))
			{
				...
				if (!((int(__cdecl*)(struct _Instance*, struct Signal*))signalInfoList[signal->id & 0x7FFFFFFF].signalHandleFunc)(instance, signal))
					return;
			}
...
{ // line 976, offset 0x8001e234
	int v2; // edi
	struct Signal* i; // esi
	void(__cdecl * signalRelocateFunc)(); // eax
		...
		i = (struct Signal*)((char*)i + 4 * signalInfoList[i->id & 0x7FFFFFFF].length + 4))
	{
		...
		if (signalRelocateFunc)
			((void(__cdecl*)(struct Signal*, int))signalRelocateFunc)(i, offset);
		++v2;
	}
...
{ // line 1049, offset 0x8001e3c0
	...
	if (LevelWithID)
	{
		v2 = *(DWORD*)(LevelWithID + 200);
		if (v2)
			COLLIDE_HandleSignal(instance, (struct Signal*)(v2 + 8), 1, 0);
	}
}
