...
struct _MonsterState monstate_tbl[31] =
{
	...
	MON_PetrifiedEntry, offset MON_Petrified
};

struct _MonsterFunctionTable mondef_tbl =
{
	MON_DefaultInit, MON_CleanUp,
	MON_DamageEffect, MonsterQuery,
	MonsterMessage, 0, __DATE__, 0
};
...
{ // line 408, offset 0x8008cf48
	...
	struct _MonsterStateChoice* stateFuncs; // ecx
	int stat; // eax
	int state; // [esp+8h] [ebp+8h]

	relocModule = (struct _MonsterFunctionTable*)instance->object->relocModule;
	if (!relocModule)
		return (TDRFuncPtr_MONTABLE_GetDamageEffectFunc)&monstate_tbl[state];
	stateFuncs = relocModule->stateFuncs;
	stat = stateFuncs->state;
	if (stateFuncs->state == -1)
		return (TDRFuncPtr_MONTABLE_GetDamageEffectFunc)&monstate_tbl[state];
	...
	return (TDRFuncPtr_MONTABLE_GetDamageEffectFunc)&stateFuncs->functions;
}
