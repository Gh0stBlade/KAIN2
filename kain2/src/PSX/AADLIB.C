...
unsigned short aadHblanksPerUpdate[4];
...
int aadFreeDynamicSfx(int handle)
{
	...
	subu    $v0, $a2
		...
		lw      $v0, 0($a1)
		nop
		bnez    $v0, loc_80052BFC
		nop
		lw      $v0, 4($a1)
		nop
		beq     $v0, $a0, loc_80052C78
		li      $v0, 2

		loc_80052BFC:
	lw      $a1, -0x6428($gp)//aadMem
		addiu   $v0, $a2, 1
		lw      $v1, 0x740($a1)
		andi    $a2, $v0, 0xF
		bne     $a2, $v1, loc_80052BCC
		sll     $v0, $a2, 3

		loc_80052C14:
	lw      $a2, -0x6428($gp)//aadMem
		nop
		lw      $v0, 0x748($a2)
		nop
		slti    $v0, 0x10
		beqz    $v0, locret_80052C84
		nop
		lw      $v1, 0x744($a2)
		nop
		addiu   $v1, -1
	...
	if (!(info->flags & 0x1))
	{
		...
		if (aadMem->numLoadReqsQueued != 0 && aadMem->sramDefragInfo.status == 0 && gDefragRequest == 0)
		{
			...
			else if (loadReq->type == 0)
			{
				...
				aadMem->nonBlockLoadProc(info->snfFileName, aadLoadDynamicSfxReturn, info, NULL, (void**)&info->snfFile, 0x2F);
	...
	else
	{
		if (info->snfFile->snfID == aadCreateFourCharID('a', 'S', 'N', 'F'))
		{
			...
			else
			{
				...
				aadMem->nonBlockBufferedLoadProc(info->smfFileName, aadLoadDynamicSfxReturn2, info, NULL);
...
{ 
	...
	else
	{
		...
		while (toneAttr->referenceCount != 0)
		{
			if (((i + 1) & 0x7F) != i)
			{
